...
在设备的实际发送和转发数据时，只识别IP地址。而IP地址不利于人类记忆，所以域名便诞生了，通常我们使用的浏览器地址栏便输入域名。

![显示错误](DNS_image/DNS1.png)

例如：

DNS便是用于将域名与IP之间进行转换的协议，存储DNS信息的服务器就叫DNS服务器。当我们需要转换域名和IP时，便会向DNS服务器进行查询。若DNS服务器有相应的条目，则会回复我们相应的信息。
...
DNS同时占用了UDP与TCP的53号端口。DNS的查询和响应通常都只使用一个数据包，UDP的报文长度限制为512字节，当域名或响应条目过长时可能会超出UDP的长度限制，这时就会使用基于TCP的DNS。由于TCP提供可靠传输，这也避免了传输途中丢包导致的无响应问题。
...
- 事务ID (Transaction ID)
  ...
  一个随机数，用于标识DNS报文的会话，对于请求报文和其对应的应答报文，该字段的值是相同的。

- 标志 (Flags)
  - QR（查询/响应标志）：`0`代表查询报文，`1`代表响应报文。
  - Opcode（操作码）：用于标识查询方式，例如标准查询、反向查询等。通常为`0`，代表标准查询。
  - AA（权威回答）：`0`代表响应服务器为非权威服务器，`1`代表响应服务器为权威服务器。
  - TC（截断）：`1`代表报文长度超过了传输协议的限制（例如UDP的512字节限制），`0`则未超过。
...
- QNAME (查询名): 要查询的域名。在编码中，会将域名的“.”替换为后面子域名的字节长度。例如，`www.google.com` 会被编码为 `3www6google3com0`，最后以0结尾。这种方式可以有效地压缩域名。
- QTYPE (查询类型): 查询的资源记录类型，例如A（1）代表IPv4地址，AAAA（28）代表IPv6地址。
- QCLASS (查询类): 查询的类别，通常为1（IN），代表互联网（Internet）。

### **Resource Record（RR，资源记录部分）**

- 资源记录部分是 `Answer`、`Authority`、`Additional` 三个部分的合称，这三个部分使用相同的报文格式。
  - Answer（回答部分）：回答部分是对问题部分的应答。
  - Authority（权威部分）
  - Additional（附加信息部分）
- 通用格式
...
TTL（生存时间）：缓存生存时间，单位为秒。表示该资源记录在本地缓存中可以保留的时间，到期后需要重新查询，这与IP头部的TTL（跳数限制）是完全不同的概念。

RDLENGTH（资源数据长度）：指定了下一个 RDATA 字段的长度，以字节为单位。

RDATA（资源数据）：记录的实际内容。其格式和含义由 TYPE 字段决定。
...
传统的DNS是明文传输的，容易导致信息泄露、劫持、篡改等问题。DoT与DoH的共同特点是使用加密传输，以保护DNS查询的隐私性和完整性。

- DoT (DNS over TLS)

  DoT技术直接将DNS的查询和应答报文封装在TLS隧道中进行传输，使用TCP 853端口。

- DoH (DNS over HTTPS)

  DoH技术将DNS查询数据打包成标准的HTTPS请求，DNS应答则封装在HTTPS的响应中返回，使用TCP 443端口。

一般来说，更加推荐使用DoH。因为HTTPS的流量非常普遍，使用DoH可以使DNS流量更好地混淆在常规网页浏览流量中，使其难以被识别和过滤。
...
CDN是一个由多地服务器组成的网络。其重点在于“分发”和“缓存”，将主服务器（源站）的数据缓存到分布在不同地点的边缘节点服务器上，从而减轻源站的负载，并让用户获得加速访问体验。
