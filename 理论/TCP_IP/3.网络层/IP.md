...
IPv4地址的类型等已在HCIA基础路由知识中阐述过了，在这便不再重复。
...
最大长度为65535（2^16-1），最小长度为20，即仅包含头部。

### **标识**

用于唯一标识一个数据包的片段组。当一个IP数据包因为太大而需要分片时，所有分片都具有相同的标识号。当主机收到数据包时，可以根据数据包的标识来重新合成分片后的数据包。
...
MTU（Maximum Transmission Unit）定义了**网络层数据单元**在数据链路层传输时所能达到的**最大长度**。如果数据包大小超过了MTU值，那么在网络层就需要对其进行**分片**处理。MTU的最大值包括IP头部的长度。
...
当TTL减为0时，路由器会丢弃该数据包，并向源IP地址发送ICMP时间超时消息（TTL传输中过期）。

利用这一特性可以实现路由跟踪技术，是网络排错的重要手段之一。
...
指示IP数据包的数据负载部分所使用的上层协议类型。这类似于TCP/IP的端口号，但不是用于为上层提供端口的，主要是用于区分协议。例如，TCP的值为6，UDP的值为17，ICMP的值为1（天字一号）。从ICMP可以窥见IP的上层协议并不一定是传输层协议。
