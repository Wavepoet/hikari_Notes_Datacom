...
SA是一个单向的安全通道，它约定了通信的双方使用的加密算法、认证算法（Hash）、密钥、生存时间以及SPI。即约定了双方应该如何通话，如何加密等。

只有协商好这些参数才能建立SA，这些参数相当于双方通信的“盟约”。
...
SA是单向的，即如果想建立一个隧道，需要建立2个SA。

- 一个用于 **出站** 流量，从本地到远端。
- 一个用于 **入站** 流量，从远端到本地。

单向性的SA使其更适合密钥管理。每个SA的密钥可以独立管理。
...
一个SA内只有一个加密密钥和认证密钥，当处理数据时，便可以缩小判断范围，以减少系统开销。

如果统一使用同一个SA进行管理，那么对数据进行处理时便要面临更多的判断，从而增加性能开销。

同时单向性的SA也会更安全。如果一个SA既负责加密发出的数据，又负责解密接收的数据，那么这个SA就需要同时持有两套密钥信息。一旦这个SA被攻破，攻击者就能同时获得双向通信的所有密钥材料。单向性的SA至少可以保障在一个SA被攻破时，攻击者也只能获得一个方向的通信数据。
...
PAD是对等体授权数据库。用来管理和验证连接对等体的身份，它里面存放着对等体的身份以及身份的验证凭据。对等体身份例如IP地址、域名等，身份的验证凭据例如数字证书、预共享密钥等。
...
在AH和ESP中有提到，由于ICV的检测机制会导致无法实现NAT穿透。而NAT-T技术可以使原本无法通过NAT转换的IPsec流量，在NAT环境下正常传输。

NAT-T即NAT穿越，它会在ESP封装之上再次封装一个UDP头，将原有的IPsec数据包隐藏在一个标准的 UDP 数据包内部。NAT-T的默认端口号是4500。

这里拿ESP举例：

![IPsecVPN1.png](images/IPsecVPN.png)

启动NAT-T后，IPsec会在IKE第一阶段的前1-2个数据包交换中协商是否支持并开启NAT-T，只有双方都开启时才能使用NAT-T。

---

## DPD（Dead Peer Detection，死对等体检测）

在标准的 IPsec 连接中，如果一个对等体突然掉线，它不会向对端发送任何通知。这会导致对端设备仍然认为对等体是活动的，它仍会向这个对等体发送数据，形成“僵尸隧道”，直到SA生命周期结束。在站点到站点的IPsec VPN中，SA生命周期通常特别长，这会造成网络资源的极大浪费。DPD正是为了解决这个问题。DPD由RFC 3706定义。

DPD 通过 IKE 交换 R-U-THERE（DPD请求）和 R-U-THERE-ACK（DPD确认）来工作。

- **DPD间隔**：发送探测包的间隔时间
- **DPD超时**：等待响应的最大时间
- **重试次数**：连续失败多少次后判定死亡

DPD可以选择按需触发（On-Demand）或者周期触发（Periodic），也可以混合两种触发方式。

- 按需触发：当IPsec本地设备准备向对等体发送数据时，会检测一定时间内有没有收到过对等体的数据包，如果没有便会发送DPD请求。
- 周期触发：无论是否有数据传输，发起方都会以固定的时间间隔发送DPD请求。

当发送DPD请求后，如果对等体回复DPD响应则正常进行传输，如果DPD超时，会在一个失败计数器上累加+1，当达到阈值时则会拆除隧道。如果配置了自动重连，它会尝试重新发起 IKE 协商，以建立一条新的 VPN 隧道。
