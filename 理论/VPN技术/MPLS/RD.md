...
运营商需要服务多个客户，这些客户可能使用的是相同的IPv4网段，这便需要RD来区分不同的IP以便BGP选路。相同RD的数据为同一VPN的数据，RD不控制VPN成员关系，不控制路由导入导出，不决定谁和谁通信。他只是用于标识唯一VPNv4路由，仅此而已。
