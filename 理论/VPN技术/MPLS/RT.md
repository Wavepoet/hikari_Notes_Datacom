...
在MPLS VPN中，PE为了隔离不同客户的路由，会为每个客户创建独立的VRF。在传递客户的路由时，便需要对路由条目进行区分，以便分辨哪条路由条目是属于哪个VRF的。
...
当对端PE收到MP-BGP路由时，会检查路由携带的RT。如果该 RT与对端某个VRF配置的Import RT完全匹配，该路由才会被允许注入到该VRF的路由表中；否则直接丢弃。在此过程中不传递 IRT，也就是说 IRT 是一个本地的 Tag，它不会被发送出去。
...
>     
> - **Extranet 场景**中，只有特定合作伙伴的 Export RT 才被允许导入，而不是所有合作伙伴的路由都混在一起。
...
传统的 MP-BGP 当中，无论有没有 RR（虽然一般都会配置 RR），路由器都会将合法的 VPNv4/v6 路由发送给所有的 MP-BGP 邻居。即使一些邻居根本不需要这些 VPN 路由，它们仍然会被发送。邻居收到后会通过 Import RT 规则进行过滤并丢弃。这显然是十分浪费资源的。
...
当 RT Filter 为全空时，即为 RT Filter 默认路由，表示对所有的 RT Tag 都感兴趣。通常用在 RR 与 RR 之间，或者 ASBR 当中。
