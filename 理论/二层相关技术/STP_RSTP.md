...
交换机在**未知目标MAC**地址时，将数据帧复制到除接收端口外的所有端口。

## **二层环路的形成**

PC向AR-1发送了一个目的地址为不存在于此网络中的IP的ARP数据包。

![image1.png](images/SRP_RSTP/image1.png)

当AR-1收到ARP包后，向除入接口外的其他接口进行泛洪，也就是向AR-2与AR-3进行泛洪。此时ARP包被复制了2份。

![image2.png](images/SRP_RSTP/image2.png)

当AR-2与AR-3收到ARP包后，又会向除入接口外的其他接口进行泛洪，可以看见ARP包又回到了AR-1，ARP包循环发送。
...
交换机根据型号而定每秒可以转发数百万、数千万个数据包（这个参数叫包转发率），如此用不了多久带宽便被环路数据包占满，老旧的交换机甚至会死机。
...
如果一个端口发出的BPDU优于接收到的BPDU，这个接口就是指定端口。
...
本机到根的路径开销，标准有802.1D与802.1T，现在主流使用802.1T。根桥的根路径开销为0，发送BPDU时保持该值，当非根交换机收到BPDU时，会提取其中的根路径开销值，转发时会加上出端口的根路径开销（802.1D为入端口）。
...
管理员手动关闭、未开启生成树，或者端口故障时的状态，此时不转发数据，不处理BPDU。
...
开始发送并接收BPDU，比较BPDU的优劣，如果选举完成后仍有角色，则进入Learning状态；如果没有角色，则退回到Blocking状态。此状态下不转发数据，不学习MAC。
...
当STP交换机检测到链路断开，会“直接变化”。如果交换机之间有设备连接链路断开，但端口正常UP，则会等到在MAX Age时间内未收到BPDU时（缺省MAX Age为20s），
...
交换机感知到拓扑变化时会生成 TCN BPDU从根端口向上游发送，逐级传递至根桥，向根桥请求TC置位，只有根桥有权限进行TC置位。

根桥收到TCN BPDU后会在 Configuration BPDU 中置位 TC，当非根桥交换机收到TC置位的BPDU后将MAC地址表的老化时间缩短为 Forward Delay（缺省为15秒），加速过期条目清除。
...
DP（指定端口）的本地备份，被本机交换机发出的更优的BPDU阻塞的端口，防止同一交换机多端口连接同一网段导致环路。

- **EP (边缘接口, Edge Port)**

旧STP当拓扑中加入新的终端设备时，接口up，就会进行重新计算STP，且链路进入转发状态需要2个Forward时间，事实上，终端设备不转发数据，所以不会形成环路。

RSTP中引入了EP（边缘端口），绕过STP计算，当接口up后可以立即进入转发状态。
...
旧STP中只有RB可以发送TC置位BPDU，RSTP所有的交换机都可以发送TC置位的BPDU。

## **检测时间加快**

STP因为早期网络质量差，在启动时需要等待30s，保证所有交换机收敛完成，实现无环。
...
P/A机制的本质便是通过握手允许DP（指定端口）快速进入转发状态，无需等待计时器，快速完成端口角色切换。

一开始每个交换机都认为自己是RB，各个接口都是DP，发送发出P置位的BPDU。

交换机发送P置位的Proposal BPDU，对端交换机接收到后与本端口保存的BPDU相比较。当确认为最优BPDU时会回复Agreement BPDU，该端口则可以立即进入转发状态。

如果收到的BPDU不是最优，但比自己优，则自己停止发送P请求。

如果收到的BPDU没有自己高，但不比自己优，则会一直发送P请求，直到30s后，仍然未发现有比自己优的，本地的端口则变成DP，对端的端口变为AP。
