...
由于网络的不断扩大，整个网络使用IGP路由便会产生大量的路由信息，并且链路状态路由协议也会透露网络的拓扑信息，对企业带来一定风险。为方便管理网络被分成了不同的AS（自治系统）。早期，EGP（外部网关协议）被用于实现在AS之间动态交换路由信息。但是EGP设计得比较简单，只发布网络可达的路由信息。BGP是为取代最初的EGP而设计的另一种外部网关协议，现在BGP已经成为唯一主流的EGP协议。
...
AS之间可能是不同机构、公司，相互之间无法完全信任，使用IGP可能存在暴露AS内部的网络信息的风险。
...
路径矢量协议相较于链路状态协议，不会发送拓扑信息；相较于距离矢量协议，它不是直接传输整个路由表，而是发送携带完整路径信息（如经过的自治系统列表）的路由条目，以此来防环并优选路由。
...
运行在同一AS内部的BGP。由于BGP为路径矢量协议，IBGP传播路由具有水平分割特性，一个AS内的对等体要建立全连接关系（Full-Mesh），需要用联盟或借助路由反射器（Route Reflector）进行路由传播。
...
RR可以把从IBGP对等体学到的路由反射（重新发送）到其他IBGP对等体的BGP设备中，类似OSPF网络中的DR。可以简化IBGP的连接，也可以通过RR传播路由使得对等体建立全连接关系。
...
它是由路由反射器创建的，这个属性带有本AS内部路由始发者的Router ID。
...
16字节，用于检测同步或身份验证，早期版本使用全1填充，BGP可能忽略此字段，因此全为F（所有比特全为1）。
...
发送此报文的路由器的AS号，通过比较My AS可以判断对等体是否属于同一AS。
...
撤回的路由。如果该值为0，表示该Update报文用于传递BGP路由。如果该值为4，表示撤销一条BGP路由（WRL=4 表示撤销4字节的路由，因为IP地址就是32bit=4字节，故表示撤销了一条路由）
...
编码格式为 <长度+前缀>
 ...
 例如192.168.1.0/24，长度为24，前缀为192.168.1，你可以直接把前缀理解为IP地址吧。
...
和BGP报头一样，但type类型标记为4，用于建立和维护对等体关系，类似OSPF的hello包。
...
BGP对等体关系建立后，BGP路由器发送BGP Update （更新）报文向对等体通告路由。
...
默认情况下，BGP使用报文出接口作为TCP连接的本地接口，当链路因为故障断开时，出接口便会down掉，BGP会话便会断掉，如果此时还有备用链路，但BGP无法使用备用链路建立对等体关系，所以部署iBGP对等体关系时，一般指定使用回环口进行TCP连接的本地地址来提高冗余性。
  ...
  root((BGP_路径属性<br/>BGP_Path_Attributes))
    ...
    Optional<br/>可选
      Optional_Transitive<br/>可选过渡
        Aggregator<br/>聚合者
        Community<br/>团体属性
      Optional_Non-transitive<br/>可选非过渡
        MED<br/>多出口鉴别器
        Cluster-List<br/>簇列表
...
- **AS_Path**
- 记录路由经过的AS序列，用于防止路由回灌产生的环路和路径选择，自己AS发出的路由不会接收，路由优选时优先选择AS_PATH最短的路由。
- 路由在被通告给EBGP对等体时，路由器会在该路由的AS_Path中追加上本地的AS号；路由被通告给IBGP对等体时，AS_Path不会发生改变。
- AS_Path又可分为AS_SEQUENCE和AS_SET两种类型。

- **Next_Hop**

用于指明到达目标网络的下一跳IP地址。

当路由器学习到BGP路由后，需要对BGP路由的Next_Hop属性进行检查，该属性值（IP地址）必须在本地路由可达，如果不可达，这条路由不可用。
...
团体属性，通过自定义标签（如**no-export**、**local-as**或私有值）可以标记一类路由，实现灵活的路由策略控制。类似IGP的tag。Community长32bit，即4Byte（字节）。
...
- MED主要用于在AS之间影响BGP的选路。MED被传递给EBGP对等体后，对等体在其AS内传递路由时，携带该MED值，但将路由再次传递给其EBGP对等体时，默认不会携带MED属性（MED不会跨越AS传递）。可使用default med命令进行修改，此命令仅对import-route引入和BGP聚合路由有效。
- 使用network和import-route方式引入路由到BGP中，产生的路由的MED值会继承IGP的metric。例如一条OSPF路由的cost值为100，将它引入BGP中时，这条路由的MED便继承为100。
...
当惩罚值低于重用阈值（Reuse Limit）时，路由将重新启用。默认重用阈值为750。
