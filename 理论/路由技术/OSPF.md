...
每个区域的路由器**只保留本区域的链路状态信息**（LSDB），这样可以使得路由器的**链路状态数据库不会过大**，拓扑变化仅影响本区域。让OSPF能很好地应用于大型网络，区域是以接口为单位的。

### **区域ID（Area ID）**

用于标识一个区域，每个OSPF区域都有一个唯一的区域ID。

可以表示成十进制数字，最大限制为4294967295（即2^32-1）
...
区域之间靠纯路由传输。没有路由算法加持，容易出现环路，所以需要一个区域作为骨干区域，所有区域必须与骨干区域相连，不同区域需要通过骨干区域进行中转，通过拓扑层面避免了环路。这种有一个中心节点（Hub）和多个边缘节点（Spokes）的架构叫**Hub-Spoke架构**（中心辐射型架构）。
...
- 用**224.0.0.5**向**所有OSPF设备**发送数据包。（所有OSPF路由器都监听224.0.0.5）
...
| Link State Request (LSR) | **链路状态请求包**，请求[LSA]，OSPF邻居双方成功交换DD报文后才会向对方发出LSR报文，请求自己没有的LSA。 |
| Link State Update（LSU） | **链路状态更新包**，回应LSR，向对端发送[LSA]，或泛洪新的LSA。内容为多条LSA。 |
| Link State Acknowledge（LSAck） | **链路状态确认包**，对LSU进行确认，靠报文中的序列号（Sequence Number）确认。 |
...
- **Checksum（校验和）：**确保数据包未被篡改过（之前讲过了不过多赘述）。
- **Authentication Type（身份验证类型）：**身份验证的类型，0为没有身份验证。
...
- **Router Dead Interval :** 死亡时间，如果在此时间内未发现邻居的hello包，则判定邻居死亡（失效），默认40s。
- **DR：** DR的接口IP地址。
- **BDR ：**BDR的接口IP地址。
...
主从选举不是DR选举，无论什么网络，路由器必须要进行主从选举。而DR选举只在BMA，NBMA网络下路由器才进行选举。
...
**LS Type（链路状态类型号）**：用于标识LSA的格式和功能。

**Link State ID（链路状态ID）**：描述需要的LSA，
...
- 网络地址一致
- 末梢区域设置一致（Option)
- OSPF建立关系不看网络类型，但由于网络类型不同导致LSA 1中的link type不同，以及LSA 2的描述不同，所以无法完成路由计算。
...
- MOSPF：（Multicast OSPF，组播扩展OSPF）， 是 OSPF 的早期组播扩展，定义于RFC 1584，但在实际部署中逐渐被 PIM（协议无关组播） 取代，因为如果使用MOSPF就必须要求整个网络中的单播路由都是OSPF的路由。
...
用于标记LSA的类型，LSA简写后面的数字就是LSA的Type，LSA 1 的LSA Type为1，LSA 2的LSA Type为2，以此类推。
...
![image17.png](images/OSPF_image/image17.png)
...
COST的默认最小单位为个位，所有小于1的数都会被约为1，可以使用命令修改，但一经修改所有的OSPF路由器都需要修改。
...
- 允许接收和传递LSA1、LSA2、LSA3，明细LSA3（允许骨干区域的LSA3进入，同时ABR会自动下发3类缺省路由。）
- 特点：
- stub不能是骨干区域
- stub不支持部署vlink（虚拟链路）
...
一种逻辑链路。因为OSPF采用了区域化的设计，并且区域也采用了Hub-Spoke的架构，所有的非骨干区域都必须与Area 0相连，当有些区域无法与Area 0直接相连时就需要用到虚链路（Virtual Link）将骨干区域的范围通过虚拟的方法扩展到相邻的非骨干区域，从而让不能直接与骨干区域相连的区域可以与骨干区域相连。

- **Virtual-link（虚链路）：**
- 是指在两台ABR之间通过一个非骨干区域建立一条逻辑上的连接通道，修复被分割的Area 0。
- 虚链路必须在两端同时配置。
- 在配置网络时应尽量减少虚链路的使用，以方便管理，网络存在永久虚链路通常表明该网络拥有缺陷。
- **Transit Area（传输区域）：**
...
在经过OSPF虚链路将Area 1也扩展为骨干区域后，可见所有的区域都变成了骨干区域，从而使网络中骨干区域能够收到另外一边被分割的骨干区域的路由（把发生“分割”的骨干区域合并），并且最后骨干区域自己的路由都为Intra-Area Route（区域内路由），但Area 1与骨干区域的路由将仍然为O IA（区域间路由）。
...
- 虚链路必须配置在两台ABR之间，这两个ABR都有一个端口与同一个非主干区域相连（只能且必须跨越一个区域），其中必须至少有一个ABR是连接骨干的。
...
- 在虚链路跨越的区域内部，通过虚链路传输的OSPF数据会以区域内数据包的形式来传输。只有在此情况下，才不要求OSPF数据包在其直连的邻居路由器之间传输。
...
SPF树中只有单向最短路径，保证了OSPF区域内不会出现环路。
