...
RIP规定超过15跳则为网络不可达。因此RIP难以适用于大型网络。
...
**水平分割**和**毒性逆转**是RIP和其他距离矢量协议中广泛运用的两大防环机制，RIP中这两个功能是默认开启的。
...
如果开启水平分割，AR-A将不会再把1.1.1.0路由条目发回给AR-B，也就不会发生路由条目错误。
...
毒性逆转相对于水平分割而言，水平分割是沉默的，而毒性逆转是主动声明不可达的，从根本上排除了回环的可能性。在一些特定的故障恢复场景中，毒性逆转的主动声明比水平分割更快地消除环路。
