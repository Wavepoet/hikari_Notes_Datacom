...
组播源需要封住报文的目标地址，包括目标IP和目标MAC。

路由器之间需要之间需要对组播报文进行路由寻址，在此需要使用到IGMP和PIM
...
IPv4的组播地址为D类的地址范围（224.0.0.0–239.255.255.255），管理这些地址分配置的组织为IANA。

IANA对组播IP地址空间进行了进一步的划分

- **范围：0.0.0–224.0.0.255**
...
- **范围：0.0.0~232.255.255.255**

SSM（Source-Specific）的临时组播地址。

- **范围：0.0.0~239.255.255.255**
...
ASM又可以分为全网MSA地址和内网ASM地址，全网ASM地址类似IP的公网地址，不可冲突，内网ASM地址类似IP的私网地址，只要不发送到公网即可。

- **SSM地址（Any-Source Multicast，指定源地址）**

接收者需同时指定组播组（G）和源地址（S），仅接收来自指定源（S）的数据。只有指定的设备都可以作为组播源，IGMPv3上开始支持SSM。
...
IGMP查询器（Querier）负责在发送查询报文，所有的IGMP都需要查询器，查询器会定期去查询是否有客户在某个组里，在一群IGMP路由器中并不需要每一台路由器都去做查询，所以需要选举出一个查询器，进行统一的查询。
...
IGMPv１使用静默离开的方式离开，离开后不再发送响应报告。

路由器普遍查询时间为６０ｓ，普遍组查询130s没有收到响应，则路由器认为该组不存在组播成员。也就是说，组播成员离开后，路由器还会发送１３０ｓ的数据，十分浪费带宽。

- **无选举机制**

IGMPV需要选举出查询器，但IGMPv１没有选举功能，只能通过ＰＩＭ协议进行选举。

## **IGMPv2**

IGMPv２相较于IGMPv１，

新增了Leave Group消息，来确定离开，如果组播成员离开，会发送一个Leave Group消息，主播源收到Leave Group消息后便会立即停止发送数据包。

新增了选举机制，不需要依靠ＰＩＭ来选举查询器。
...
查询者的健壮变量，如果不为0，QRV中包含中一个被查询者使用的[健壮变量]的值，如果查询者的健壮变量的值超过7，即QRV字段的最大值，那么QRV被设成0。路由器取最近收到的查询中的QRV值作为它们自己的健壮性变量的值，除非最近收到的QRV是0，在这种情况下，接收者使用缺省的健壮性变量值，或者是一个静态配置的值
...
开启IGMP Snooping ，交换机会监听IGMP协议报文，哪个接口收到igmp repoet即和该组播组有关系，构建 **组播成员表**，记录哪些端口的主机加入了特定组播组，当收到组播数据时只从有关系的组播接口发出。

## **IGMP之间版本差异**

| 机制 | IGMPv1 | IGMPv1 | IGMPv1 |
| --- | --- | --- | --- |
...
如名字一样，PIM不像MOSPF，DVMRP一样需要依靠单播协议进行组播路由表的构建，PIM依靠单播路由构建组播路由表，这得以让PIM成为主流。

要使用PIM组建组播路由表需要单播先能通。

目前PIM有两种模式—PIM-DM和PIM-SM。

- **PIM路由表**

PIM了路由表叫MRI（组播路由信息库），往下，PIM的转发包含（S，G） 和 （*，G）两种类形，
...
当组播源发送组播数据时，第一跳路由器会将其泛洪到所有接口上。第二路由器收到后也会泛洪到所有接口上，这个过程会逐级进行，直到数据包到达网络中的所有 PIM 路由器，每次转发前会进行RPF检测。
...
RP选举可以静态指定RP和动态RP，手动配置静态RP， RP不会选举BSP，当RP故障时，容易导致组播瘫痪。
...
MDT可分为STP（源树，最短路径树）和RPT（汇集点树，共享树）

RPT即以RP为根，客户到RP的路径，STP即以源为根，源到RP的路径，选举出DR后即可开始建立RPT和SPT。

- 1，选举DR（Designated Router，指定路由器）

PIM 路由器之间周期性发送 PIM Hello，默认为间隔为30s，通过DR Priority字来选举DR，
...
STP和RTP构建的树都经过RP，但这可能不是客户到源的最佳线路，Switchover机制既是解决这一问题。

一开始客户不知道源在哪，需要找到RP，再由RP转到源。

当源和客户都找到RP，RP告诉了源和客户的地址，这即意味着源和客户知道了对方的地址，通单播路由表即可知道最佳路径，不再需要去找RP。
